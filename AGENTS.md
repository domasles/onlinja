...
- **Typography Engine Rules:** Fonts must map explicitly to pre-compiled native utility handles configured via explicit global CSS class hooks. Aggressive font weights or style synthesis modifiers must not be combined dynamically at runtime over base family strings; instead, dedicated heavy font assets must be applied through targeted design system utility targets (`className="font-logo"`, `className="font-subheader"`, etc.).
...
- **Header Structure:** The brand title is rendered in a prominent, heavy, geometric display face using the explicit `font-logo` wrapper, positioned cleanly beneath an inline dual-circle icon component representing contrasting game tokens. Taglines underneath are styled using the clean `font-desc` handle to preserve sentence formatting layout room.
- **Segmented Control Sliders:** Configuration selectors (Strategic/Aggressive modes, White/Black sides) are laid out via flat, custom `ActionSlider` components managing active option highlights locally, styling segment subheaders with `font-subheader`.
- **Action Triggers:** Primary buttons span the full width of the configuration card, utilizing high-contrast solid fills, sharp text mapped via the `font-button` helper class, and slightly rounded clean edges (`rounded-xl`).
...
- **Scoreboard Frame:** Positioned cleanly at the top of the viewport axis (`ScoreHeader`), rendering current point aggregates with `font-score`, active player turn markers with `font-status`, and active match phase numbers with `font-label` single-color typographic parameters.
- **Interactive Token Nodes:** Individual game pieces are rendered as simple, solid or hollow monochrome circle buttons. Visual selection and history state tracking are represented exclusively through high-contrast colored borders (`border-amber-400` for Phase 1 historical tracks, `border-emerald-400` for Phase 2 historical tracks, and scaled thick borders for active selections). Home-lane structures visually condense adjacent pieces of the same color into a single merged token node carrying a numeric indicator text (`+count`) configured via `font-status` down to a ceiling bounded by `config.maxLaneCapacity`.
- **Turn Event Interrupt Overlays:** When an extra turn condition is met, a localized, absolute-positioned container intercepts user input (`pointerEvents` or explicit blocker buttons consuming event propagation). It applies a subtle backdrop blur (`backdrop-blur-[4px]`) and displays a centered, perfectly aligned geometric vector graphic indicator consisting of a standalone `+` sign mapped via nested layout views inside a rounded white border container, accompanied by standard `font-desc` structured validation typography detailing the active player state to mimic standard system notification alerts.
...
- **Universal Turn Forfeiture (Vulnerability Trap):** If the Phase 1 initiative move lands on a lane containing 0 other pieces prior to the move (resulting in a final landing lane occupant count of exactly 1), or lands directly inside the opponent's home base storage, the active player instantly forfeits the remainder of their turn. This forfeiture rule applies universally to all turns, including active extra-turn phases. Phase 2 validation is bypassed, zero momentum steps are awarded, and the active player state swaps immediately to the opponent.
...
- **Home Lane Clustering and Visual Aggregation:** To honor `config.maxLaneCapacity` bounds inside the terminal home bases without dropping tracking elements, adjacent consecutive tokens matching identical side alignments are merged programmatically into static sub-clusters. If a player token moves into a lane occupied by opposing forces, the own pieces condense leftwards into a composite indicator node displaying a combined quantitative counter (`+count`), preserving the exact position matrix of arriving alternative pieces inside a fixed layout space tracking up to 6 total elements.
...
- **The Empty Lane Privilege:** If a Phase 2 momentum move terminates on a destination lane that contained exactly 0 pieces, an extra turn sequence is awarded to the active player. This evaluation must target a frozen state snapshot recorded immediately prior to the execution of Phase 1 of the active player's turn sequence. A lane becoming empty solely because a piece departed from it during the current macro-move cycle does not satisfy this condition. An extra turn cannot chain recursively; landing on a separate empty lane during an active extra turn will not trigger a new extra turn multiplier and will conclude the player's turn sequence.
